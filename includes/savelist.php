...
{	
	...
	$name = $_POST['listName'];
	$stmt = $db->prepare("INSERT into checklist (Name) VALUES (?)");
	$stmt->execute(array($name));
	
	//Finds the Id of the newly created list
	$stmt = $db->prepare("SELECT checklist.Id FROM checklist WHERE checklist.Name = '$_POST[listName]'");
	if($stmt->execute())
	{
	...
	$stmt = $db->prepare("INSERT into checkmapping (checkId, checklistId, CheckNumber) VALUES (?, ?, ?)");
